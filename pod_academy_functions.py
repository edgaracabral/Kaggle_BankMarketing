...
print('-----------------------------------------------------')
print('Atualização: 1.0.2')
print('Aluno: Edgar Cabral')
print('Data: 06/12/2023')
print('-----------------------------------------------------')
...
def avaliar_modelo_squad2_plot2(X_train, y_train, X_test, y_test, modelo, nm_modelo, fs_x, fs_y):
    import matplotlib.pyplot as plt
    import pandas as pd
    import numpy as np
    import itertools
    ...
    feature_names = list(X_train.columns)
    # Criação da figura e dos eixos
    # fig, axs = plt.subplots(3, 2, figsize=(15, 18))  # Ajustado para incluir novos gráficos
    #fig, axs = plt.subplots(1, 3, figsize=(12, 3.5))  # Ajustado para incluir novos gráficos
    fig, axs = plt.subplots(1, 3, figsize=(fs_x, fs_y))  # Ajustado para incluir novos gráficos
    ...
    y_score_train = modelo.predict_proba(X_train)[:, 1]
    fpr_train, tpr_train, _ = roc_curve(y_train, y_score_train)
    axs[0].plot(fpr_train, tpr_train, color=cor, label='Treino')

    y_score_test = modelo.predict_proba(X_test)[:, 1]
    fpr_test, tpr_test, _ = roc_curve(y_test, y_score_test)
    axs[0].plot(fpr_test, tpr_test, color='darkorange', label='Teste')

    axs[0].plot([0, 1], [0, 1], color='navy', linestyle='--')
    axs[0].set_title('ROC Curve - Treino e Teste - ' + nm_modelo)
    ...
    ks_train = calcular_ks_statistic(y_train, y_score_train)
    ks_test = calcular_ks_statistic(y_test, y_score_test)
    axs[1].bar(['Treino', 'Teste'], [ks_train, ks_test], color=[cor, 'darkorange'])
    axs[1].set_title('KS - ' + nm_modelo)
    axs[1].set_ylim(0, 1)
    axs[1].text('Treino', ks_train + 0.01, f'{ks_train:.2f}', ha='center', va='bottom')
    axs[1].text('Teste', ks_test + 0.01, f'{ks_test:.2f}', ha='center', va='bottom')
    ...
    deciles = pd.qcut(scores, q=10, duplicates='drop')
    decile_analysis = y_test.groupby(deciles).mean()
    axs[2].bar(range(1, len(decile_analysis) + 1), decile_analysis, color='darkorange')
    axs[2].set_title('Ordenação do Score - Teste - ' + nm_modelo)
    axs[2].set_xlabel('Faixas de Score')
    axs[2].set_ylabel('Taxa de Evento')

    # Mostrar os gráficos
    plt.show()


def avaliar_modelo_squad2_plot1(X_train, y_train, X_test, y_test, modelo, nm_modelo, fs_x, fs_y):
    import matplotlib.pyplot as plt
    import pandas as pd
    import numpy as np
    import itertools
    ...
    feature_names = list(X_train.columns)
    # Criação da figura e do tamanho
    fig, axs = plt.subplots(figsize=(fs_x, fs_y))  # Ajustado para incluir novos gráficos
    plt.tight_layout(pad=6.0)
    ...
    if importancias is not None:
        ...
        axs.barh(importancias_df['feature'], importancias_df['importance'], color=cor)
        axs.set_title('Importância das Variáveis - ' + nm_modelo)
        axs.set_xlabel('Importância')

    else:
        axis('off')  # Desativa o subplot se não houver importâncias para mostrar
